...
macro cartesian(x::Union{Vector,Symbol,Expr}, z::Union{Vector,Symbol,Expr})
    return esc(:(@.($z * sin($x)), @.($z * cos($x))))
end

macro inn(expr)
    return esc(:(@views $expr[2:(end - 1)]))
end
...
function init_annulus(nθ::Int64, nr::Int64; spacing=20)
...
function primary_grid(nθ, nr, r_out)
    rl = R .- [20.0f0, 35.0f0, 210.0f0, 410.0f0, 660.0f0, 2740.0f0, 2891.5f0]
    ...
    # -- Nodal positions
    r_column = vcat(rl, LinRange(r_in, r_out, nr - length(rl)))
    sort!(r_column)
    ...
    gr = Grid2D(x, z, θ, r, connectivity, nθ, nr, nel, length(x), neighbours, element_type)
...
function velocity_layers(nθ, spacing)
    ...
    r = R .- (20.0f0, 35.0f0, 210.0f0, 410.0f0, 660.0f0, 2740.0f0, 2891.5f0)
...
function add_mid_points(l::Tuple{AbstractArray,AbstractArray}, spacing)
    ...
    mid_θ = reduce(
        vcat,
        LinRange(l[1][i], l[1][i + 1], Int(abs(l[1][i] - l[1][i + 1]) ÷ deg))[2:(end - 1)]
        for i in 1:(n - 1)
    )
    mid_θ = vcat(
        mid_x, LinRange(l[1][end - 1], 2π, Int(abs(l[1][end - 1] - 2π) ÷ deg))[2:(end - 1)]
    )
...
function constrain2layers!(G::Dict, gr)
    ...
    @inbounds for (i, Gi) in G
        ...
        elseif ri == rlayer[end]
            upper_limit, lower_limit = rlayer[end - 1], 0.0f0
...
@inbounds function find_layer(ri, rlayer)
    ri < rlayer[end] && return (rlayer[end], 0.0f0)
...
function incidence_matrix(gr)
    ...
    return incidence_matrix = sparse(J, I, V)
end
...
function unconstrained_element_incidence(gr)
    ...
    return incidence_matrix = sparse(I, J, V)
end
...
function edge_connectivity(gr)
    ...
    @inbounds for iel in 1:nel
        ...
        edge_neighbours = [
            map_edges(e2n, i, map_triangle, map_rectangle) for i in el_neighbours
        ]
...
function secondary_nodes(gr; spacing=20)
    ...
    center = closest_point(gr, 0.0f0, 0.0f0; system=:polar) # id of node at the center of the universe
...
@generated function distance(a::NTuple{N,T}, b::NTuple{N,T}) where {N,T}
    quote
        Base.@_inline_meta
        d = zero(T)
        Base.Cartesian.@nexprs $N i -> d += (a[i] - b[i])^2
        return √d
    end
end
...
@inline function distance(a::Point2D{Cartesian}, b::Point2D{Cartesian})
...
function closest_point(gr, px, pz; system=:cartesian)
    ...
    v_x, v_z = ifelse(system == :cartesian, (gr.x, gr.z), (gr.θ, gr.r))
...
function velocity_layers(gr, nθ, spacing)
    ...
    for iel in gr.nel
        ...
        if check # break element in two
            ...
            min_θ, max_θ = extrema((
                gr.θ[element[1]], gr.θ[element[2]], gr.θ[element[3]], gr.θ[element[4]]
            ))
...
function isinlayer(element, rl) # make a @generated function
    min_r, max_r = extrema((
        gr.r[element[1]], gr.r[element[2]], gr.r[element[3]], gr.r[element[4]]
    ))
