...
function triangle_annulus_2D(; 
    ...
    Delauney2D(x, z, θ, r, e2n, neighbours_list, Int32(size(e2n,2)), Int32(length(x)))
end
...
function layered_domain(xs, zs, max_area)
    ...
    depth_list =  6371f0.-Float32.([20, 35, 210, 410, 660, 2740, 2891.5, 5153.5])
    ...
    #          xr1,          zr1, id1, elsize
    region1 = [x1[1]*0.99, z1[1],   1, 1f0]
    region2 = [x2[1]*0.99, z2[1],   2, 1f0]
    ...
    region_list = hcat(region1, region2, region3, region4, region5, region6, region7, region8)
...
distance(ax::T, az::T, bx::T, bz::T) where T<:Number = √(muladd(az - bz, az - bz, (ax - bx) * (ax - bx)))

distance_from_center(x::T, z::T) where T<:Number = √(x*x + z*z)

cart2polar(x::T, z::T) where T <:Real = (
    atan(x, z),
    distance_from_center(x, z)
)
...
function add_midpoints(e2n, neighbours_list, x::Vector{T}, z::Vector{T}; npoints = 1) where T
