...
function grid(c0, c1, nnods)
    ...
    return Grid{LinearMesh,eltype(c0),eltype(nels)}(
        c0, c1, nels, nnods, nnods[1] * nnods[2], x, y, z
    )
end
...
function lazy_grid(c0, c1, nnods)
    ...
    return LazyGrid(c0, c1, (Δx, Δy, Δz), nels, nnods, nnods[1] * nnods[2])
end
...
function getindex(gr::Grid, I::Int, J::Int, K::Int)
    ...
    @assert J <= gr.nnods[2]
    @assert K <= gr.nnods[3]
    return Point(gr.x[I], gr.y[J], gr.z[K])
end
...
function cornerindex_ijk(gr::Grid{LinearMesh,M,N}, iel::Int) where {M,N}
    return (
...
function distance3D(ax::T, ay::T, az::T, bx::T, by::T, bz::T) where {T<:Real}
